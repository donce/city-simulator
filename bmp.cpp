...
AUX_RGBImageRec::AUX_RGBImageRec(const char *FileName): data(0), NoErrors(false)
{ 
 loadFile(FileName);
}
...
bool AUX_RGBImageRec::loadFile(const char* Filename)
{
	...
	if(data != 0) delete data;					// windows needs this info to determine what header info we are looking for  
	...
	GetDIBits(gldc, (HBITMAP)DIBHandle, 0,0, 0, &BMInfo, DIB_RGB_COLORS);
